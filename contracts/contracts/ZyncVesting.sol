...
///      cannot be created, so every beneficiary can always be paid.
...
contract ZyncVesting is Ownable, ReentrancyGuard {
    ...
    function release(uint256 scheduleId) external nonReentrant {
        ...
        uint256 releasableAmount = _vestedAmount(s, block.timestamp) - s.released;
        if (releasableAmount == 0) revert NothingToRelease();

        // EFFECTS first: mark released and free the commitment before transferring.
        s.released += releasableAmount;
        totalCommitted -= releasableAmount;

        // INTERACTION last.
        token.safeTransfer(msg.sender, releasableAmount);
        emit Released(msg.sender, scheduleId, releasableAmount);
    }
